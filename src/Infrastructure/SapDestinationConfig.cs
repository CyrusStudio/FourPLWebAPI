...
public class SapDestinationConfig : IDestinationConfiguration
{
    ...
    public RfcConfigParameters GetParameters(string destinationName)
    {
        ...
        {
            // 取得目標環境 (DEV/QAS/PRD)
            var sapEnv = _configuration.GetValue<string>("SapEnvironment") ?? "DEV";
            
            // 從 Sap:{環境} 區段讀取設定
            var sapSection = _configuration.GetSection($"Sap:{sapEnv}");
            
            // 若找不到該環境設定，嘗試使用舊格式 (直接從 Sap 讀取)
            if (!sapSection.Exists())
            {
                sapSection = _configuration.GetSection("Sap");
            }
